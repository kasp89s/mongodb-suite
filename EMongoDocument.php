...
abstract class EMongoDocument extends EMongoEmbeddedDocument {
	 ...
	 * 
	 * @return string array name, or array of fields for composite primary key
	 * @since v1.2.2
	 */
	 ...
	 * 
	 * @param boolean $flag
	 *        	true|false value for FSync flag
	 */
	 ...
	 * 
	 * @param boolean $flag
	 *        	true|false value for Safe flag
	 */
	 ...
	 *        	true|false value for use cursor flag
	 */
	...
	private function ensureIndexes() {
		Yii::trace ( get_class ( $this ) . '.ensureIndexes()', 'ext.MongoDb.EMongoDocument' );
		...
		foreach ( $this->indexes () as $name => $index ) {
			if (! in_array ( $name, $indexNames )) {
				if (version_compare ( Mongo::VERSION, '1.0.2', '>=' ) === true) {
					$this->getCollection ()->ensureIndex ( $index ['key'], array (
							'unique' => isset ( $index ['unique'] ) ? $index ['unique'] : false,
							'name' => $name 
					) );
				} else {
					$this->getCollection ()->ensureIndex ( $index ['key'], isset ( $index ['unique'] ) ? $index ['unique'] : false );
				}
	...
	public function applyModifiers(&$modify){
		if ($modify === null) {
			$modify = new EMongoModifier();
		} else if (is_array ( $modify )) {
			$modify = new EMongoModifier ( $modify );
		} else if (! ($modify instanceof EMongoModifier))
			throw new EMongoException ( 'Cannot apply modify to criteria' );
		
		if (($c = $this->getModifier(false)) !== null) {
			$c->mergeWith ( $modify );
			$modify = $c;
			$this->_modifier = null;
		}
	 ...
	 *        	meaning all attributes that are loaded from DB will be saved.
	 * @param
	 *        	boolean modify if set true only selected attributes will be replaced, and not
	 *        	the whole document
	...
	public function update(array $attributes = null, $modify = false) {
		...
		if ($this->beforeSave ()) {
			...
			Yii::trace ( get_class ( $this ) . ".update() attributes:" . var_export ( $attributes, true ), 'ext.MongoDb.EMongoDocument' );
			Yii::log ( get_class ( $this ) . ".update() Raw Data\r\n" . var_export ( $rawData, true ), 'debug' );
			// filter attributes if set in param
			if ($attributes !== null) {
			...
			Yii::log ( get_class ( $this ) . '.update() Processed Data\r\n' . var_export ( $rawData, true ), 'debug' );
			if ($modify) {
				if (isset ( $rawData ['_id'] ) === true)
					unset ( $rawData ['_id'] );
				
				$result = $this->getCollection ()->update ( array (
						'_id' => $this->_id 
				), array (
						'$set' => $rawData 
				), array (
	 ...
	 * The return type of update changed in 1.09 if you are using safe => TRUE
	 * <pre>
	 * array(
	 * [err]=>					//error
	 * [updatedExisting]=>1,	//If an upsert updated an existing element
	 * [n]=>1,					//documents effects
	 * [upserted]=>_id			//the new record's _id field.
	 * )
	 * <pre>
	 *
	 * @since v1.3.6
	 * @param EMongoModifier $modifier
	 *        	updating rules to apply
	 * @param EMongoCriteria $criteria
	 *        	condition to limit updating rules
	 * @param
	 *        	array options for mongodb fsync,safe,upsert,multiple
	 * @return bool array
	 */
	public function updateAll($modifier, $criteria = null, $options = array()) {
		...
		if ($modifier->canApply === true) {
			...
			Yii::trace ( get_class ( $this ) . '.updateAll()' . print_r ( $criteria->getConditions (), true ) . print_r ( $modifier->getModifiers (), true ), 'ext.MongoDb.EMongoDocument' );
			if (! isset ( $options ['fsync'] ))
				$options ['fsync'] = $this->getFsyncFlag ();
			if (! isset ( $options ['safe'] ))
				$options ['safe'] = $this->getSafeFlag ();
			if (! isset ( $options ['upsert'] ))
				$options ['upsert'] = false;
			if (! isset ( $options ['multiple'] ))
				$options ['multiple'] = true;
			
			$result = $this->getCollection ()->update ( $criteria->getConditions (), $modifier->getModifiers (), $options );
			return $result;
		} else {
			return false;
		}
	 ...
	 *
	 *
	 * Update fields of current model
	 *
	 * @param EMongoModifier|array $modifier        	
	 * @param Boolean $upsert        	
	 * @throws EMongoException
	 */
	public function updateSelf($modifier = NULL, $options = array()) {
		$options ['multiply'] = false;
		if ($modifier == null) {
			if ($this->_modifier == null)
				throw new EMongoException ( 'No modifier found' );
			$modifier = $this->_modifier;
			$this->_modifier = null;
		} else {
			if ($this->_modifier != null) {
				$this->_modifier->mergeWith ( $modifier );
				$modifier = $this->_modifier;
				$this->_modifier = null;
			}
		}
	 ...
	 * @param array|EMongoCriteria $condition
	 *        	query criteria.
	 * @since v1.0
	 */
	...
	public function refresh() {
		...
		if (! $this->getIsNewRecord () && $this->getCollection ()->count ( array (
				'_id' => $this->_id 
		) ) == 1) {
			$this->setAttributes ( $this->getCollection ()->find ( array (
					'_id' => $this->_id 
			) ), false );
			return true;
		} else
	...
	public function find($criteria = null) {
		...
		if ($this->beforeFind ()) {
			...
			$selects = $criteria->getSelect ();
			Yii::trace ( get_class ( $this ) . '.find()' . print_r ( $conditions, true ) . print_r ( $selects, true ), 'ext.MongoDb.EMongoDocument' );
			$doc = $this->getCollection ()->findOne ( $conditions, $selects );
	 ...
	 * @param bool $upsert
	 */
	public function findAndModifyNative($criteria = null,$modifier = null,$remove=null, $update=null,$new=null,$upsert = null){
		Yii::trace ( get_class ( $this ) . '.findAndModifyNative()', 'ext.MongoDb.EMongoDocument' );
		if ($this->beforeFind ()) {
			$this->applyScopes ( $criteria );
			...
			if($criteria && $criteria->getSort()!=array()){
				$options['sort']=$criteria->getSort();
			}
			if($criteria && $criteria->getSelect()!=array())
				$fields=$criteria->getSelect();
			Yii::trace ( get_class ( $this ) . '.findAndModifyNative()' . print_r ( $conditions, true ) . print_r ( $fields, true ), 'ext.MongoDb.EMongoDocument' );
			$obj =  $this->getCollection()->findAndModify($conditions,$modifier->getModifiers(),$fields,$options);
			return $obj;
	 ...
	 * @return EMongoDocument
	 */
	public function findAndModify($criteria = null,$modifier = null,$remove=null, $update=null,$new=null,$upsert = null){
		$obj=$this->findAndModifyNative($criteria,$modifier,$remove,$update,$new,$upsert);
		if($obj===array())
			return null;
		if($new && $remove===false)
			return $this->populateRecord($obj);
		if(isset($obj['value']))
			return $this->populateRecord($obj['value']);
		return $this->populateRecord($obj);
	}
	...
	public function findAll($criteria = null) {
		Yii::trace ( get_class ( $this ) . '.findAll()' . var_export ( $criteria, true ), 'ext.MongoDb.EMongoDocument' );
	 ...
	 *        	whathever do a case-sensitive search, default to false
	 * @return EMongoDocument
	 * @since v1.2.2
	 */
				...
				if (is_array ( $this->$attribute ) || is_object ( $this->$attribute ))
					...
					if ($op !== '')
						call_user_func ( array (
								$criteria,
								$attribute 
						), $op, is_numeric ( $value ) ? floatval ( $value ) : $value );
					else
	 ...
	 *
	 *
	 * Get Modifier of current model
	 *
	 * @param Boolean $createIfNull        	
	 *
	 * @return EMongoModifier instance of modifier
	 */
	public function getModifier($createIfNull = true) {
		if ($this->_modifier === null)
			if ($createIfNull)
				$this->_modifier = new EMongoModifier ();
	...
	/*public function __set($name, $value) {
		if($this->_logModify===true){
			$this->getModifier()->addModifier($name, 'set', $value);
		}
		return parent::__set($name, $value);
	}
	
	public function beginLogModify(){
		$this->_logModify=true;
	}
	
	public function stopLogModify(){
		$this->_logModify=false;
	}*/
